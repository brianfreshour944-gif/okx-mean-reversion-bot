...
def run():
    ...
    while True:
        try:
            # 1. Heartbeat
            db.execute_query("INSERT INTO bot_status (bot_name, last_update, status) VALUES (%s, NOW(), 'RUNNING') ON CONFLICT (bot_name) DO UPDATE SET last_update = NOW()", ('okx-mean-reversion-bot
',))
